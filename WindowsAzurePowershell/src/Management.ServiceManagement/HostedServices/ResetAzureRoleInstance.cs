...
namespace Microsoft.WindowsAzure.Management.ServiceManagement.HostedServices
{
    ...
    public class ResetAzureRoleInstanceCommand : ServiceManagementBaseCmdlet
    {
        ...
        private void RebootSingleInstance(string instanceName)
        {
            ...
            {
                ExecuteClientAction(null, CommandRuntime.ToString(), s => this.Channel.RebootDeploymentRoleInstanceBySlotTask(s, this.ServiceName, this.Slot, instanceName), WaitForOperation);
            }
        }
        ...
        private void ReimageSingleInstance(string instanceName)
        {
            using (new OperationContextScope(Channel.ToContextChannel()))
            {
                ExecuteClientAction(null, CommandRuntime.ToString(), s => this.Channel.ReimageDeploymentRoleInstanceBySlotTask(s, this.ServiceName, this.Slot, instanceName), WaitForOperation);
            }
        }
