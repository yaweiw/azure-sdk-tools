...
namespace Microsoft.WindowsAzure.Management.Storage.Test.Blob.Cmdlet
{
    ...
    public class SetAzureBlobContentTest : StorageBlobTestBase
    {
        ...
        public void OnFinishTest()
        {
            ...
            command.OnFinish(pr, null);
            Assert.AreEqual(100, pr.PercentComplete);
            Assert.AreEqual(String.Format(Resources.UploadFileSuccessfully, ""), pr.StatusDescription);
            command.OnFinish(pr, e);
            Assert.AreEqual(100, pr.PercentComplete);
            Assert.AreEqual(String.Format(Resources.UploadFileFailed, "", e.Message), pr.StatusDescription);
        }
