...
namespace Microsoft.WindowsAzure.Commands.CloudService.Development
{
    ...
    public class EnableAzureServiceProjectRemoteDesktopCommand : CmdletBase
    {
        ...
        public void EnableRemoteDesktop()
        {
            ...
            CloudServiceProject service = new CloudServiceProject(GeneralUtils.GetServiceRootPath(CurrentPath()), null);
        ...
        private static string GetForwarderName(WebRole[] webRoles, WorkerRole[] workerRoles)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        firstWorkerRole.Imports = GeneralUtils.Append(firstWorkerRole.Imports, new Import { moduleName = "RemoteForwarder" });
                    ...
                    else // no worker role, use a web role
                    {
                        ...
                        {
                            firstWebRole.Imports = GeneralUtils.Append(firstWebRole.Imports, new Import { moduleName = "RemoteForwarder" });
        ...
        private static void AddRemoteAccess(WebRole[] webRoles, WorkerRole[] workerRoles)
        {
            ...
            {
                webRole.Imports = GeneralUtils.Append(webRole.Imports, new Import { moduleName = "RemoteAccess" });
            }
            foreach (WorkerRole workerRole in workerRoles.Where(r => r.Imports == null || !r.Imports.Any(i => i.moduleName == "RemoteAccess")))
            {
                workerRole.Imports = GeneralUtils.Append(workerRole.Imports, new Import { moduleName = "RemoteAccess" });
            }
        }
