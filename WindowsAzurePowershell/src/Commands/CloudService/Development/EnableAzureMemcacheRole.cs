...
namespace Microsoft.WindowsAzure.Commands.CloudService.Development
{
    ...
    public class EnableAzureMemcacheRoleCommand : CmdletBase
    {
        ...
        {
            string rootPath = GeneralUtils.GetServiceRootPath(CurrentPath());
            RoleName = string.IsNullOrEmpty(RoleName) ? GeneralUtils.GetRoleName(rootPath, CurrentPath()) : RoleName;

            EnableAzureMemcacheRoleProcess(this.RoleName, this.CacheWorkerRoleName, GeneralUtils.GetServiceRootPath(CurrentPath()));
        }
