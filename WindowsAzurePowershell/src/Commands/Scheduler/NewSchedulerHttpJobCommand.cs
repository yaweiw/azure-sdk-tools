...
namespace Microsoft.WindowsAzure.Commands.Scheduler
{
    ...
    public class NewSchedulerHttpJobCommand : SchedulerBaseCmdlet
    {
        ...
        [Parameter(Position = 3, Mandatory = true, ValueFromPipelineByPropertyName = false, ParameterSetName = RequiredParamSet, HelpMessage = "The Method for Http and Https Action types (GET, PUT, POST, HEAD or DELETE).")]
        [ValidateSet("GET", "PUT", "POST", "HEAD", "DELETE", IgnoreCase = true)]
        public string Method { get; set; }
        
        [Parameter(Position = 4, Mandatory = true, ValueFromPipelineByPropertyName = false, ParameterSetName = RequiredParamSet, HelpMessage = "The Uri for job action.")]
