...
namespace Microsoft.WindowsAzure.Management.CloudService.Test.Tests
{
    ...
    public class AddAzurePHPWebRoleTests : TestBase
    {
        ...
        public void SetupTest()
        {
            ...
            addPHPWebCmdlet.CommandRuntime = mockCommandRuntime;
        ...
        public void AddAzurePHPWebRoleProcess()
        {
            using (FileSystemHelper files = new FileSystemHelper(this))
            {
                ...
                addPHPWebCmdlet.AddAzurePHPWebRoleProcess(roleName, 1, rootPath);

                AzureAssert.ScaffoldingExists(Path.Combine(files.RootPath, "AzureService", roleName), Path.Combine(Resources.PHPScaffolding, Resources.WebRole));
                Assert.AreEqual<string>(roleName, Testing.GetPSVariableValue<string>((PSObject)mockCommandRuntime.WrittenObjects[1], Parameters.RoleName));
                Assert.AreEqual<string>(expectedVerboseMessage, mockCommandRuntime.VerboseChannel[1]);
            }
        }
    }
}
